...
begin 
    using  Roots ,PlutoUI,HypertextLiteral, ExtendableGrids, VoronoiFVM, PlutoVista,GridVisualize,LinearAlgebra
...
# ╔═╡ 4bb9ef58-29a2-446b-afe3-405363e31b6d
begin
	fpeakue(x)=0
	fpeakv(x)=2*u0+2
end
...
function flux(f,u,edge)
	...
	f[2]=-(sigma_i*(u[1,1]-u[1,2])+sigma_i*(u[2,1]-u[2,2])+sigma_e*(u[2,1]-u[2,2]))
	f[3]=0
end
...
begin
    ...
    inival[1,:].=map(fpeaku,grid)
	inival[2,:].=map(fpeakue,grid)
	inival[3,:].=map(fpeakv,grid)
